...
import { getBed, updateBed } from '../../services/bedService'
import { listRooms } from '../../services/roomService'

export default function BedEdit() {
    const { id } = useParams()
    const navigate = useNavigate()
    const bed = useMemo(() => getBed(id), [id])
    ...
    if (!bed) {
        return (
            <div className="p-8">
                <p className="text-gray-600">Bed not found.</p>
                <Link to="/owner/beds" className="text-blue-600 hover:underline">Back to beds</Link>
            </div>
        )
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <h1 className="text-3xl font-semibold text-gray-900">{bed.number}</h1>
                </div>
